...
bool check(pair<int, int> cur, int t) {
	...
	if (G[nxt.first][nxt.second] == '#')
		return false;
	else if (G[nxt.first][nxt.second] == '.')
		return true;
	else if (G[nxt.first][nxt.second] == '[')
		return check(nxt, t) && check(nxt + d[0], t);
	else
		return check(nxt, t) && check(nxt + d[2], t);
}
...
signed main() {
	...
	for (char c : moves) {
		if (c == '>' || c == '<') move(cur, dir[c]);
		if ((c == '^' || c == 'v') && check(cur, dir[c])) move(cur, dir[c]);
